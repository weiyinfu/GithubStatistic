...
class GithubCrawler {
    ...
    static exist(username, callback) {
        axios.get(`http://github.com/${username}`).then(resp => callback(true)).catch(err => {
            callback(false)
        })
    }
    ...
    crawl(url) {
        if (this.visited.has(url)) {//已访问过的不必再访问
            return
        }
        ...
        axios.get(url).then((resp) => {
            ...
            this.activeCount--
            this.cout("请求失败")
            if (err.response)//如果是其它地方的异常，可能没有response对象，防止再出异常
